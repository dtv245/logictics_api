...
public class Load extends BaseAuditableEntity {
    ...
    /**
     * Transitions the Load to {@link LoadStatus#DISPATCHED}.
     * Valid only if current state is DRAFT.
    ...
    public void dispatch() {
        ...
        if (!LoadStatus.isValidTransition(current, LoadStatus.DISPATCHED)) {
            throw new InvalidStateTransitionException("Load",
                    current != null ? current.dbValue() : "null",
    ...
    /**
     * Transitions the Load to {@link LoadStatus#PICKED_UP}.
     * Valid only if current state is DISPATCHED.
     * Sets {@code pickedUpAt} timestamp if null.
     *
     * @throws InvalidStateTransitionException if transition is not allowed
     */
    public void pickUp() {
    ...
    /**
     * Transitions the Load to {@link LoadStatus#DELIVERED}.
     * Valid only if current state is PICKED_UP.
    ...
    /**
     * Transitions the Load to {@link LoadStatus#CANCELLED} from any non-terminal state.
     * Sets {@code cancelledAt} timestamp if null.
     *
     * @throws InvalidStateTransitionException if the Load is already in a terminal state (DELIVERED or CANCELLED)
     */
    public void cancel() {
        var current = getStatusEnum();
    ...
    /**
     * Calculates the driver share for this Load.
     * Ratio = sum of salaryAmount of assigned truck's mainDriver and secondaryDriver
     *         where salaryType = "ShareOfGross"
     * DriverShare = deliveryCostAmount × ratio
     * <p>
     * If ratio > 1, throws IllegalStateException (data integrity violation).
     * If no ShareOfGross driver, ratio = 0 and driverShare = 0.
     *
    ...
    public BigDecimal calcDriverShare() {
        ...
        if (ratio.compareTo(BigDecimal.ONE) > 0) {
            throw new IllegalStateException(
                    "Driver share ratio " + ratio + " exceeds 1.0 for Load " + id
                    + ". Check salary configuration of drivers assigned to truck " + assignedTruck.getId());
        }

        return deliveryCostAmount.multiply(ratio);
    }
}