 ...
 *
 * <p>The {@code toView} method maps the claims list via expression to produce {@link
 * RoleView.ClaimView} objects.
 */
...
public interface RoleMapper {
  ...
  @Mapping(
      target = "claims",
      expression =
          "java(role.getClaims() == null ? java.util.List.of() : role.getClaims().stream().map(c -> new com.company.logicstic.modules.role.dto.RoleView.ClaimView(c.getId(), c.getClaimType(), c.getClaimValue())).toList())")
