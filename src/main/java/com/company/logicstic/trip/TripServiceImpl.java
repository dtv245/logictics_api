...
import com.company.logicstic.notification.event.TenantNotificationEvent;
...
public class TripServiceImpl extends AbstractBaseService<Trip, TripResponse, CreateTripRequest>
  ...
  public TripServiceImpl(
    ...
    super(tripRepository, tripMapper::toResponse, tripMapper::toEntity, tripMapper::updateEntity);
  ...
  private void resolveRelations(Trip trip, CreateTripRequest req) {
    if (req.truckId() != null) {
      trip.setTruck(truckService.getEntityById(req.truckId()));
    } else {
      trip.setTruck(null);
    }
  }
  ...
  private TripResponse saveAndNotify(Trip trip) {
    ...
    request.stops().stream()
        ...
        .forEach(
            item -> {
              ...
              var load = loadService.getEntityById(item.loadId());
