...
import com.company.logicstic.dto.TenantMigrationReport;
    ...
    public TenantMigrationReport migrateAllActiveTenants() {
        ...
        for (int index = 0; index < tenants.size(); index++) {
            ...
            try {
                ...
                List<String> pendingTenants = tenants.subList(index, tenants.size()).stream()
                        .map(TenantRegistryRecord::tenantId)
                        .toList();
                log.error("Tenant migration failed for tenant={}. Migrated tenants={}, pending tenants={}",
                        tenant.tenantId(), migratedTenants, pendingTenants, ex);
                return TenantMigrationReport.failed(
                        List.copyOf(migratedTenants),
                        pendingTenants,
                        tenant.tenantId(),
                        ex.getMessage());
            }
