...
class TenantRoutingDataSourceTests {
  ...
  void tenantAQueriesDoNotReturnTenantBData() {
    ...
    List<String> tenantAResults =
        tenantJdbcTemplate.queryForList("SELECT name FROM customers", String.class);

    TenantContext.setTenantId("tenant-b");
    List<String> tenantBResults =
        tenantJdbcTemplate.queryForList("SELECT name FROM customers", String.class);
  ...
  private void seedCustomer(HikariDataSource dataSource, String customerName) {
    ...
    jdbcTemplate.execute(
        "CREATE TABLE customers (id UUID PRIMARY KEY, name VARCHAR(100) NOT NULL)");
