package com.company.logicstic.modules.inspection;
...
class InspectionDefectsTest {
  ...
  void createAndUpdateRebuildDefectsWithBidirectionalOwnership() {
    ...
    InspectionService service =
        new InspectionService(
            reports,
            findByIdRepository(LoadRepository.class, load),
            findByIdRepository(EmployeeRepository.class, inspector),
  ...
  private static <T> T findByIdRepository(Class<T> type, Object entity) {
    return proxy(
        type,
        (method, args) -> {
          if (method.equals("findById")) {
