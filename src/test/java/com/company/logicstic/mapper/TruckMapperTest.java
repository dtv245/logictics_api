...
/**
 * Pure unit tests for {@link TruckMapper} using Mappers.getMapper().
 * No Spring context required — avoids DB dependency.
 * Verifies parity with the old {@code TruckView.from()} logic.
 */
class TruckMapperTest {
    ...
    /**
     * CreateTruckRequest: number, type, vehicleCapacity, status, make, model, year,
     * vin, licensePlate, licensePlateState, isHazmatPlacarded, mainDriverId, secondaryDriverId,
    ...
    void toEntity_setsDefaultsForNullBooleans() {
        CreateTruckRequest req = new CreateTruckRequest(
                "TRK-003", "REEFER", 22000, "ACTIVE",
                "Freightliner", "Cascadia", 2022,
                "VIN123", "ABC-123", "CA",
                null,  // isHazmatPlacarded → defaults to false
                null, null,   // mainDriverId, secondaryDriverId
    ...
    void toView_matchesOldFromMethodOutput() {
        ...
        TruckView expected = new TruckView(
                truckId, "T-100", "TANKER", 30000, "ACTIVE",
                "Volvo", "VNL", 2021, "VIN999", "XYZ-789", "TX",
                true,
                main.getId(), "Tom Brown",
                sec.getId(), "Sue Green",
                true, "2,3", 51.5, -0.1
        );
